...
//read a nonempty line from file
bool readFromFile(fstream &file,string &content){
	content.clear();
	string tmp;	
	do{
		if(!getline(file,tmp))
		...
		/*
		if(tmp.size()!=0){
			int p=tmp.find_first_not_of(" 	");
			tmp=tmp.substr(p,tmp.find_last_not_of(" 	")-p+1);
		}
		*/
		content+=tmp;
	}while(tmp.find(";")==(size_t)(~0));	
	return true;


}

//devide line into many options
bool takeOptions(const string &line, vector<string> &options){
	options.clear();
	string tmp;
